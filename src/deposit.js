...
useEffect(()=>{
   const fetchdata=async()=>{
      await axios.get('https://back-end-server-qggz.onrender.com/data').then((item)=>{setData(item.data)})
   };fetchdata()
},[]);
...
function handleClick(e) {
  ...
  if (user) {
    ...
    axios.put(`https://back-end-server-qggz.onrender.com/update/${user._id}`, { amount: updatedAmount })
      .then(() => {
